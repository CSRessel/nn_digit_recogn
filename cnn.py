...
lnoises = sys.argv[1]
lnoise = float(lnoises)
snoise = float(sys.argv[2])
...
# Convolutional Layer #2
  ...
  dense = tf.layers.dense(inputs=pool3_flat, units=750, activation=tf.nn.relu)
...
def main(unused_argv):
  ...
  num_noisy_labels = num_samples * lnoise
  ...
  for i in garbled_labels:
    ...
    inc_labels.remove(train_labels[i])
    train_labels[i] = random.choice(inc_labels)
  ...
  mnist_classifier = tf.estimator.Estimator(
      model_fn=cnn_model_fn, model_dir="convnet_model"+lnoises)#, config=cfg)
